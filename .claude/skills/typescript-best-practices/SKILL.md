...
description: Use this skill whenever generating any typescript file. Provides TypeScript patterns for type-first development, making illegal states unrepresentable, exhaustive handling, and runtime validation. Must use when reading or writing TypeScript/JavaScript files.
---
...
- Always define and use types whenever necessary.
  **IMPORTANT**
- NEVER USE `any` type
- NEVER USE `unknown` type

## Pair with React Best Practices

When working with Typescript files use this skill. This skill covers TypeScript fundamentals;
...
- Wrap all async operations in `try/catch` — never let errors silently fail
- Always log with contextual info using the project logger:

```ts
import logger from "@conservice/components/logger";

// Good — includes context
logger.error("Failed to fetch company details", { error, companyId });

// Bad
console.error(error);
```

- Implement error boundaries in React components for unexpected rendering errors
- Return `null` or a typed fallback from actions on failure; never throw to the UI layer
- For async server actions, always wrap with `try/catch` and log before returning `null`:

```ts
export const getMyResource = async (id: string): Promise<MyResponse | null> => {
  try {
    const response = await get(`/api/my-resource/${id}`);
    return MyResponseSchema.parse(response);
  } catch (error) {
    logger.error("Failed to fetch my resource", { error, id });
    return null;
  }
};
```
